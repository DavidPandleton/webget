...
class TestParseOpts:
    def test_positional(self):
        ...
        assert retry_transient is False and engine is None
    ...
    def test_limit(self):
        *_, limit, _, _, _, _, _, _, _ = opts("s", "q", "--limit", "7")
        assert limit == 7

    def test_profile_and_no_cache(self):
        *_, profile, no_cache, _, _, _, _ = opts("u", "https://x.com", "--profile", "campus")
        assert profile == "campus" and no_cache is False
        *_, profile, no_cache, _, _, _, _ = opts("u", "https://x.com", "--no-cache")
        assert profile is None and no_cache is True

    def test_strategy(self):
        *_, strategy, _, _, _, _, _, _ = opts("u", "https://x.com", "--strategy", "crawl4ai")
        assert strategy == "crawl4ai"

    def test_concurrency(self):
        *_, concurrency, _, _ = opts("u", "https://x.com", "--concurrency", "5")
        assert concurrency == 5

    def test_headless(self):
        *_, headless, _, _, _ = opts("login", "https://x.com", "--headless")
        assert headless is True

    def test_retry_flag(self):
        *_, retry_transient, _ = opts("u", "https://x.com", "--retry")
